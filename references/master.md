...
- **项目实践记忆**：每次开始实质性项目工作前，主动读取当前项目的 `docs/ata/master_practices.md`。它只记录当前项目中已经提炼并仍然有效的 master 实践。
...
2. 如果用户需求模糊、抽象或存在关键分歧，主动启用 `grilling` 这个skill，一次聚焦一个问题，不断对用户进行追问，直到与用户的想法对齐，足以编写计划。
3. 编写计划或文档之前，读取 `references/docs.md`，了解文档编写要求和规范。
4. 确认当前项目根目录，检查 `docs/ata/master_practices.md`；存在时先读取，不存在时按照 `references/docs.md` 的项目实践记忆规范创建空文档后再继续。
...
2. 如果下一阶段目标仍模糊、存在多种路线或用户意图未确认，先和用户商讨；必要时启用 `grilling`，不要用阶段计划替用户做产品决定。
...
5. 判断 worker 提出的经验或文档建议属于项目事实、代码规范、角色实践候选还是无需记录的临时信息；角色实践候选写入当前项目对应的 `docs/ata/notes/`，不直接追加到实践手册。
...
1. 当实际协作中出现可能值得复用、但暂时无法确认是否应成为稳定实践的 master 经验时，简要写入当前项目的 `docs/ata/notes/master.md`。
2. notes 是候选观察，不是当前规则；普通任务不读取 notes 指导行动，也不要求在写入时完成最终判断。
3. 当 notes 积累到需要整理、同类问题反复出现或阶段收口时，安排专门的提炼任务：合并重复内容、删除偶发噪声，把仍然有效的项目级 master 经验重写进 `docs/ata/master_practices.md`，并清理已经处理的 notes。
4. 如果提炼结果属于所有项目都应遵守的 aTa 角色边界，提出对 aTa 的修改建议；如果属于项目技术事实或代码规范，写入对应项目事实源，不留在角色实践手册中。
